...
def index():
    ...
    if request.method == 'POST':
        ...
        for i in username:
            if i.isspace():
                username_error = 'Your username must not contain spaces.'
                username = ''
            else:
        ...
        if (not password_error) and (not username_error) and (not email_error) and (not verify_password_error):
            return redirect('/confirmedpage?username={0}'.format(username))
...
@app.route('/confirmedpage')
def confirmedpage():
    title = "Welcome New User!"
    username = request.args.get('username')
    return render_template('confirmed.html', title=title, username=username)
